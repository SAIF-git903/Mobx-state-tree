...
const TodoView = () => {
    ...
    return (
        <div>
                    ...
                    <div style={{ display: "flex", gap: "30px" }}>
                        ...
                        <div className='align-centered' style={{ gap: "10px" }}>
                                ...
                                <div className='align-centered' style={{ gap: "10px", flexWrap: "wrap" }}>
                                    {
                                        store?.users?.map((user) => {
                                            return (
                                                <div className='centered assignee-hover-effect' style={{ backgroundColor: "#eeecec", padding: "0 20px", borderRadius: "10px" }} onClick={() => handleAssigneeClick(user)}>
                                                    <UserOutlined />
                                                    <h5>{user.name}</h5>
                                                </div>
                                            )
                            ...
                            <Dropdown menu={{ items, onClick }}>
                                {
                                    isPriority ? <FlagFilled className='centered flag-icon-props' style={{ backgroundColor: priorityNum === "1" ? "red" : priorityNum === "2" ? "goldenrod" : "#36a836", color: "white" }} /> : <FlagOutlined className='centered flag-icon-props' />
                                }
                            </Dropdown>
                            <Button
                                type="primary"
                                style={{ backgroundColor: priorityNum === "1" ? "red" : priorityNum === "2" ? "goldenrod" : "#36a836", }}
                                onClick={handleAddTodo}>Add To do</Button>
                        </div>
                    </div>
                </div>
            </div>
            <div style={{ display: "flex", justifyContent: "flex-end", marginRight: "30px", gap: "30px" }}>
            ...
            {
                !searchInp.length > 0 && <div style={{ display: "flex", justifyContent: "flex-end", marginRight: "30px" }}>
                    <Select
                        mode="multiple"
            ...
            {
                searchInp.length > 0 ? searchedTodos.map((todo, index) => {
                    return (
                        <TodoList todo={todo} key={index} users={users} />
                    )
                }) : filteredData ? filteredData.map((todo, index) => {
                    return (
                        <TodoList todo={todo} key={index} users={users} />
                    )
